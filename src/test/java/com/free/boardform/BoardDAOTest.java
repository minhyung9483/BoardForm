...
public class BoardDAOTest {
	...
	/*
	 * @Test public void testGetBoardList() throws Exception { List<BoardVO>
	 * boardList = boardDAO.getBoardList(); logger.info("\n Board List \n ");
	 * if(boardList.size() > 0) { for(BoardVO list : boardList) {
	 * logger.info(list.title); } } else { logger.info("데이터가 없습니다."); } }
	 */
	...
	public void testGetBoardContent() throws Exception {
		...
		if(boardVO != null) {
			...
			logger.info("카테고리 : " + boardVO.getCate_cd() );
	...
	public void testInsertBoard() throws Exception {
		 ...
		 * logger.info("\n Insert Board Result " +result); if(result == 1) {
		 * logger.info("\n 게시물 등록 성공 "); } else { logger.info("\n 게시물 등록 실패"); }
		 */
	}
