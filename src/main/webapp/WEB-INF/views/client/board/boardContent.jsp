...
<script>
	...
	function showReplyList(){
		...
		var paramData = {"bid" : "${boardContent.bid}"}; //게시글 아이디
		$.ajax({
            type: 'POST',
            url: url,
            data: paramData,
            ...
            success: function(result) {
               	var htmls = "";
			if(result.length < 1){			
				$("#replyList").html("<div>등록된 댓글이 없습니다.</div>");
			} else {
                    $(result).each(function(){
                     ...
                     htmls += '<span style="padding-left: 7px; font-size: 9pt">';
                     htmls += '<a href="javascript:void(0)" onclick="fn_editReply(' + this.rid + ', \'' + this.reg_id + '\', \'' + this.content + '\' )" style="padding-right:5px">수정</a>'; //댓글 수정
                     htmls += '<a href="javascript:void(0)" onclick="fn_deleteReply(' + this.rid + ')" >삭제</a>'; //댓글 삭제
                     htmls += '</span>';
                     htmls += '</span>';
                     htmls += this.content;
                     htmls += '</p>';
